...
public class Requisito7 {
	...
	@Test // Requisito 7
	public void idUnicoParaNFs() {
		//facadenf = FacadeBDNotaFiscal.getInstance(bdnf);
		NFNaoFinalizada nf = new NFBuilder().adicionarItem(3, "Bulldog").build();
		NFNaoFinalizada nf2 = new NFBuilder().adicionarItem(4, "Bulldog").build();
		...
		Mockito.when(bdnf.validar(nf2)).thenReturn(true);
		Mockito.when(bdnf.gerarProxID()).thenReturn(2);
		NFFinal nfvalidada2 = validador.validarNotaFiscal(nf2);
		
		assertEquals(nfvalidada1.getID(), 1);
		assertEquals(nfvalidada2.getID(), 2);
	}
}
